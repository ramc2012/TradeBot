...
export default function CommodityLivePage() {
  ...
  // IST ISO `time`; trades carry `exit_time`. Prefer the backend's
  // pre-split `today_trades` when present, else filter trade_history by
  // exit date. Undated orders are kept (defensive — never hide a live order).
  ...
  // / per-underlying are lifetime aggregates and would be meaningless if
  // scoped to a single day. Only the Trades + Orders LISTS are today-only.
  ...
  // P&L semantics (2026-06-02): the backend `day_pnl` is now ALREADY
  // realized-today + open MTM, and `day_realized_pnl` is the realized-today
  // component on its own. The old UI read `day_pnl` (then realized-only) and
  // manually added `unrealized` — now that double-counts. Use the backend
  // fields directly; fall back to the manual sum only if an old backend
  // (no day_realized_pnl) is ever served. `realized_pnl_lifetime` is the
  // total realized since the paper account opened (what the user asked to
  // see alongside Day P&L).
