...
/**
 * Dealer-positioning analytics panel for the long-premium (directional) module.
 *
...
function Tile({ label, value, detail, color }: { label: string; value: string; detail?: string; color?: string }) {
  return (
    <div className="rounded-2xl border border-bg-border bg-bg-secondary/35 px-4 py-3">
      ...
      <div className={clsx("mt-2 font-mono text-lg font-semibold text-text-primary", color)}>{value}</div>
    ...
    return (
      <Section title="Dealer positioning · GEX" icon={<Sigma size={16} />}>
        <div className="rounded-2xl border border-bg-border bg-bg-primary/16 p-4 text-sm text-text-muted">
          No chain cached for <span className="font-mono">{underlying}</span>. GEX analytics populate once the
          broker websocket fills the option chain. Open Market → Option Chain once for this underlying to prime it.
        </div>
      </Section>
    );
  }
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="ml-auto text-[11px] text-text-muted">
          spot {num(m.spot, 1)} · fwd {num(m.fp, 1)} · {m.days != null ? `${m.days}d` : "—"}
        </span>
      </div>
      ...
      <Section title={`Dealer positioning · ${underlying} ${activeExpiry ?? ""}`} icon={<Sigma size={16} />}>
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
          <Tile
            ...
            detail={m.net_gex != null ? (m.net_gex >= 0 ? "long-gamma · stabilizing" : "short-gamma · trending") : undefined}
          />
          <Tile
            label="Gamma flip"
            value={m.gamma_flip != null ? num(m.gamma_flip, 0) : "—"}
          ...
          />
          <Tile label="ATM IV" value={m.atm_iv != null ? `${m.atm_iv.toFixed(2)}%` : "—"} />
          <Tile label="PCR (OI)" value={m.pcr != null ? m.pcr.toFixed(3) : "—"} color={pcrTone(m.pcr)} />
          <Tile
            label="Max pain"
            value={num(m.max_pain, 0)}
            detail={m.spot != null && m.max_pain != null ? `${(((m.max_pain - m.spot) / m.spot) * 100).toFixed(2)}% vs spot` : undefined}
          />
          <Tile
            label="Net DEX"
            value={`${m.net_dex != null && m.net_dex >= 0 ? "+" : ""}${compact(m.net_dex)} Cr`}
            color={tone(m.net_dex)}
            detail={`walls C${num(m.call_wall, 0)} / P${num(m.put_wall, 0)}`}
          />
        </div>
      </Section>
        ...
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={gexData} margin={{ top: 6, right: 8, bottom: 4, left: 4 }}>
              ...
              <Tooltip
                formatter={(v: number) => [`${compact(v)} Cr`, "GEX"]}
                labelFormatter={(s) => `Strike ${s}`}
                contentStyle={CHART_TT}
              />
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.label} className="border-b border-bg-border/20">
                ...
                <td className="px-2 py-1.5 text-right font-mono">{num(r.max_pain, 0)}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    ...
    >
      {!open ? (
        ...
        <div className="rounded-2xl border border-bg-border bg-bg-primary/16 p-4 text-[12px] text-text-muted">
