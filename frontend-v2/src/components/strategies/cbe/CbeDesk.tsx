...
export default function CbeDesk() {
  ...
  return (
    <DeskShell
      ...
      description={`Compression-Before-Expansion long/short book — weekly alpha engine (MACD · RSI · RRG). ${
        scan?.source ?? ""
      }`}
      ...
      {activeTab === "performance" ? (
        <div className="space-y-4">
          <section className="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-6">
            ...
            <MetricTile label="Available" value={formatSignedMoney(markedPaperSum?.available_capital)} detail={`reserved ${formatSignedMoney(markedPaperSum?.reserved_margin)}`} />
          </section>
...
function markOpenPositions(
  ...
  return positions.map((position) => {
    ...
    const currentPnl = (mark - entry) * qty * cbeSideSign(position);
...
function markPaperSummary(summary: PaperSummary | undefined, open?: MarkedCbePosition[]): PaperSummary | undefined {
  ...
  if (grossRatio > maxGross + 0.0001) riskFlags.push("gross_exposure_over_mandate");
  if (Math.abs(netRatio) > maxNet + 0.0001) riskFlags.push("net_exposure_over_mandate");
  if (largestRatio > maxSingle + 0.0001) riskFlags.push("single_name_over_mandate");
  if (topSectorRatio > maxSector + 0.0001) riskFlags.push("sector_concentration_over_mandate");
                ...
                return (
                  <tr key={p.position_id ?? `${side}-${i}`} className="border-b border-bg-border/20 last:border-b-0">
                    <td className="px-3 py-2">
                      ...
                      <div className="text-[10px] text-text-muted">{p.mark_source ?? "mark"}{p.mark_scan_date ? ` · ${p.mark_scan_date}` : ""}</div>
                    </td>
