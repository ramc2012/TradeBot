...
export default function InstitutionalConvergenceDesk() {
  ...
  return <DeskShell title="Institutional Convergence" description={`Staged 3-minute ${market} setup · 2-of-3 flow confirmation · anti-chase paper execution`} asOf={latest?.generated_at} isFetching={query.isFetching || run.isPending} paperMode tabs={TABS} activeTab={activeTab} onTabChange={setActiveTab}
    ...
    {data && activeTab === "orderflow" ? <div className="space-y-4">
      ...
      <Section title="Aggression, initiative & absorption" icon={<Activity size={16}/>} description="Signed aggressive volume by three-minute bar; pressure beyond ±60% marks initiative, while amber points flag high-volume absorption."><OrderFlowPulse bars={selectedFull?.footprint?.bars} source={selectedFull?.footprint?.source ?? selectedFull?.cvd?.source} asOf={selectedFull?.footprint?.bars?.at(-1)?.time ?? latest?.generated_at} dataMode={flowProvenance.dataMode}/></Section>
      <div className="grid gap-4 xl:grid-cols-[1.15fr_.85fr]"><Section title="Recent 3-minute footprints" icon={<BookOpen size={16}/>} description="Executed volume reconstructed at price; ≥3× imbalance is highlighted."><FootprintGrid bars={selectedFull?.footprint?.bars} source={selectedFull?.footprint?.source} maxBars={4} dataMode={flowProvenance.dataMode}/></Section><Section title="Option OI walls" icon={<Layers3 size={16}/>}><WallPanel result={selectedFull}/></Section></div>
