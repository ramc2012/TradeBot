  ...
  return <DeskShell title="Institutional Convergence" description={`Staged 3-minute ${market} setup · 2-of-3 flow confirmation · anti-chase paper execution`} asOf={latest?.generated_at} isFetching={query.isFetching || run.isPending} paperMode tabs={TABS} activeTab={activeTab} onTabChange={setActiveTab}
    ...
    {data && activeTab === "profile" ? <div className="space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
        <Section title="Current TPO profile" icon={<BarChart3 size={16}/>} rightSlot={<LastUpdated timestamp={latest?.generated_at} label="scan"/>}><MarketProfileChart profile={selectedFull?.profile} lastPrice={selectedFull?.spot} height={420}/></Section>
      ...
      <Section title="Three-minute price path" icon={<Activity size={16}/>}><PriceChart result={selectedFull}/></Section>
    </div> : null}
    ...
    {data && activeTab === "orderflow" ? <div className="space-y-4">
      ...
      <LiveOrderFlowTape symbol={(selectedFull?.futures_contract?.includes(":") || selectedFull?.futures_contract?.includes("|")) ? selectedFull.futures_contract : underlyingToTapeSymbol(selectedFull?.symbol) ?? selectedFull?.symbol} title={`${selectedFull?.symbol ?? market} · live microstructure`} />
      <section className="grid grid-cols-2 gap-3 md:grid-cols-6"><MetricTile label="Stage" value={selectedFull?.setup_state ?? "LEGACY"} detail={selectedFull?.preferred_direction ? `${selectedFull.preferred_direction} candidate` : "awaiting next staged scan"}/><MetricTile label="Confirmations" value={selectedFull?.confirmation_count != null ? `${selectedFull.confirmation_count}/${selectedFull.confirmation_required ?? 2}` : "—"} detail={selectedFull?.quality ?? "legacy snapshot"}/><MetricTile label="CVD" value={selectedFull?.cvd?.divergence?.kind ?? "impulse/none"} detail={selectedFull?.cvd?.source ?? "—"}/><MetricTile label="Ticks" value={String(selectedFull?.footprint?.tick_count ?? 0)}/><MetricTile label="Buy ratio" value={`${formatNumber(selectedFull?.footprint?.long_ratio, 2)}×`}/><MetricTile label="Sell ratio" value={`${formatNumber(selectedFull?.footprint?.short_ratio, 2)}×`}/></section>
      <Section title="Price versus cumulative volume delta" icon={<Waves size={16}/>}><CvdPanel series={selectedFull?.cvd?.series} source={selectedFull?.cvd?.source} divergence={selectedFull?.cvd?.divergence} height={300}/></Section>
      <Section title="Aggression, initiative & absorption" icon={<Activity size={16}/>} description="Signed aggressive volume by three-minute bar; pressure beyond ±60% marks initiative, while amber points flag high-volume absorption."><OrderFlowPulse bars={selectedFull?.footprint?.bars} source={selectedFull?.footprint?.source ?? selectedFull?.cvd?.source} asOf={selectedFull?.footprint?.bars?.at(-1)?.time ?? latest?.generated_at}/></Section>
      <div className="grid gap-4 xl:grid-cols-[1.15fr_.85fr]"><Section title="Recent 3-minute footprints" icon={<BookOpen size={16}/>} description="Executed volume reconstructed at price; ≥3× imbalance is highlighted."><FootprintGrid bars={selectedFull?.footprint?.bars} source={selectedFull?.footprint?.source} maxBars={4}/></Section><Section title="Option OI walls" icon={<Layers3 size={16}/>}><WallPanel result={selectedFull}/></Section></div>
