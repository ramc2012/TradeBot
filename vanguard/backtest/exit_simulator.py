...
def r_multiple(entry_premium: float, exit_price: float) -> float | None:
    ...
    R = (exit - entry) / entry -- i.e. the unit is the FULL premium paid, not
    the stop distance. That is the honest denominator for a long option: the
    position can go to zero and the stop is a courtesy the market may gap
    straight through (see _fillable). M8 previously divided by
    ...
    NOTE (2026-08-27): this is now a RETURN ON PREMIUM, not a multiple of the
    sized risk. Migration 006 moved M7's sizing basis to risk-at-stop, so
    `tickets.sizing_risk_rupees` and this R no longer share a denominator and
    `pnl_rupees / sizing_risk_rupees` is a different (larger) number. That is
    deliberate: R stays a property of the TRADE, comparable across every
    sizing regime the lane has ever run, while sizing_risk_rupees is a
    property of the POSITION. Anything that needs risk-multiples should divide
    pnl by sizing_risk_rupees explicitly and say so.
